library(tidyverse)
library(readr)
m <- read_csv("Tasks/aLook/matrix.csv", col_types = cols(ID_user = col_character()))
...
m <- m %>% 
  mutate(month=as.integer(last_txn_days/31), 
         ...
         false_neg = churn == 0 & pred == 1, 
         false_pos = churn == 1 & pred == 0,
         true_pos = churn == 1 & pred == 1,
         true_neg = churn == 0 & pred == 0,
...
test <- m %>% group_by(month, pred, churn) %>% summarise(mean(total)) %>% filter(churn==1) %>% filter(pred==0)
test 
